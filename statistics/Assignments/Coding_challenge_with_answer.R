####### Coding challenge
head(iris)
### Question 1: Means of Sepal.length of setosa differs from means of versicolor. Create a graph
# to examine the difference between two groups. 
library(tidyverse)
library(rstatix)
library(ggpubr)
...
iris %>% 
  filter(Species != "virginica") %>%
  group_by(Species) %>%
  ggqqplot(x = "Sepal.Length", facet.by = "Species")
